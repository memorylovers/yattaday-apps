  ...
  group('AccountStore', () {
    ...
    group('リアルタイム更新', () {
      test('Firestoreの変更が自動的に反映される', () async {
        ...
        final initialAccount = await container.read(
          accountStoreProvider.future,
        );
        expect(initialAccount, isNotNull);

        // Firestoreのデータを更新
        final updatedData =
            initialAccount!.copyWith(updatedAt: DateTime.now()).toJson();

        await fakeFirestore.collection('accounts').doc(uid).update(updatedData);

        // 少し待機してリアルタイム更新を待つ
        await Future.delayed(const Duration(milliseconds: 100));

        // 更新が反映されていることを確認
        final updatedAccount = await container.read(
          accountStoreProvider.future,
        );
        expect(
          updatedAccount!.updatedAt,
          isNot(equals(initialAccount.updatedAt)),
        );
      });
    });
    ...
    group('myAccountProvider', () {
      test('accountStoreProviderの値を返す', () async {
        ...
        final accountFromStore = await container.read(
          accountStoreProvider.future,
        );
        final accountFromMyAccount = await container.read(
          myAccountProvider.future,
        );
    ...
    group('認証状態の変更', () {
      test('ログアウト時にアカウント情報がクリアされる', () async {
        ...
        final accountAfterLogout = await container.read(
          accountStoreProvider.future,
        );
