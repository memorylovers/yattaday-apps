...
class Translations implements BaseTranslations<AppLocale, Translations> {
	...
	late final TranslationsRecordItemsEn recordItems = TranslationsRecordItemsEn.internal(_root);
}
...
class TranslationsPaymentEn {
	...
	late final TranslationsPaymentHeaderEn header = TranslationsPaymentHeaderEn.internal(_root);
	String currentPlan({required Object plan}) => 'Current Plan: ${plan}';
	late final TranslationsPaymentPlansEn plans = TranslationsPaymentPlansEn.internal(_root);
	late final TranslationsPaymentPriceEn price = TranslationsPaymentPriceEn.internal(_root);
	late final TranslationsPaymentButtonsEn buttons = TranslationsPaymentButtonsEn.internal(_root);
}
...
class TranslationsRecordItemsEn {
	...
	String get retry => 'Retry';
	String navigateToDetail({required Object title}) => 'Navigate to ${title} details';
	String navigateToEdit({required Object title}) => 'Navigate to edit ${title}';
	String get navigateToCreate => 'Navigate to create';
	String deleteConfirm({required Object title}) => 'Show delete confirmation for ${title}';
	String get edit => 'Edit';
	String get delete => 'Delete';
	String get notFound => 'Record item not found';
	String get deleteTitle => 'Delete Record Item';
	String get deleteConfirmation => 'Are you sure you want to delete this record item?';
...
class TranslationsPaymentPlansEn {
	...
	// Translations
	late final TranslationsPaymentPlansFreeEn free = TranslationsPaymentPlansFreeEn.internal(_root);
	late final TranslationsPaymentPlansStandardEn standard = TranslationsPaymentPlansStandardEn.internal(_root);
	late final TranslationsPaymentPlansPremiumEn premium = TranslationsPaymentPlansPremiumEn.internal(_root);
}
...
class TranslationsPaymentPlansFreeEn {
	...
	late final TranslationsPaymentPlansFreeFeaturesEn features = TranslationsPaymentPlansFreeFeaturesEn.internal(_root);
}
...
class TranslationsPaymentPlansStandardEn {
	...
	late final TranslationsPaymentPlansStandardFeaturesEn features = TranslationsPaymentPlansStandardFeaturesEn.internal(_root);
}
...
class TranslationsPaymentPlansPremiumEn {
	...
	late final TranslationsPaymentPlansPremiumFeaturesEn features = TranslationsPaymentPlansPremiumFeaturesEn.internal(_root);
}
...
extension on Translations {
	dynamic _flatMapFunction(String path) {
		switch (path) {
			...
			case 'settings.planManagement': return 'Plan Management';
			case 'settings.currentPlan': return 'Current Plan';
			case 'settings.free': return 'Free';
			case 'settings.profileEdit': return 'Edit Profile';
			case 'settings.appSettings': return 'App Settings';
			case 'settings.notifications': return 'Notifications';
			case 'settings.notificationsDesc': return 'Customize reminder notifications';
			case 'settings.theme': return 'Theme';
			case 'settings.lightMode': return 'Light Mode';
			case 'settings.language': return 'Language';
			case 'settings.japanese': return 'Japanese';
			case 'settings.support': return 'Support';
			case 'settings.help': return 'Help';
			case 'settings.contactUs': return 'Contact Us';
			case 'settings.terms': return 'Terms of Service';
			case 'settings.privacy': return 'Privacy Policy';
			case 'settings.logout': return 'Logout';
			case 'settings.userName': return 'Taro Tanaka';
			...
			case 'payment.selectPlan': return 'Select Plan';
			case 'payment.header.title': return 'Unlock More Features';
			case 'payment.header.subtitle': return 'Make your daily records even better';
			case 'payment.currentPlan': return ({required Object plan}) => 'Current Plan: ${plan}';
			case 'payment.plans.free.name': return 'Free';
			case 'payment.plans.free.features.items': return 'Record items: Up to 3';
			case 'payment.plans.free.features.basicRecording': return 'Basic recording features';
			case 'payment.plans.free.features.simpleStats': return 'Simple statistics';
			case 'payment.plans.standard.name': return 'Standard';
			case 'payment.plans.standard.features.items': return 'Record items: Unlimited';
			case 'payment.plans.standard.features.detailedStats': return 'Detailed stats & graphs';
			case 'payment.plans.standard.features.dataExport': return 'Data export';
			case 'payment.plans.standard.features.reminder': return 'Reminder features';
			case 'payment.plans.premium.name': return 'Premium';
			case 'payment.plans.premium.features.allStandard': return 'All Standard features';
			case 'payment.plans.premium.features.aiAnalysis': return 'AI analysis & advice';
			case 'payment.plans.premium.features.teamShare': return 'Team sharing';
			case 'payment.plans.premium.features.prioritySupport': return 'Priority support';
			case 'payment.price.perMonth': return '/month';
			case 'payment.buttons.currentPlan': return 'Current Plan';
			case 'payment.buttons.selectPlan': return 'Select This Plan';
			case 'payment.buttons.recommended': return 'Recommended';
			...
			case 'recordItems.title': return 'Record Items';
			case 'recordItems.empty': return 'No record items';
			case 'recordItems.searchPlaceholder': return 'Search';
			case 'recordItems.searchNotAvailable': return 'Search feature is coming soon';
			case 'recordItems.errorMessage': return 'An error occurred';
			case 'recordItems.retry': return 'Retry';
			case 'recordItems.navigateToDetail': return ({required Object title}) => 'Navigate to ${title} details';
			case 'recordItems.navigateToEdit': return ({required Object title}) => 'Navigate to edit ${title}';
			case 'recordItems.navigateToCreate': return 'Navigate to create';
			case 'recordItems.deleteConfirm': return ({required Object title}) => 'Show delete confirmation for ${title}';
			case 'recordItems.edit': return 'Edit';
			case 'recordItems.delete': return 'Delete';
			case 'recordItems.notFound': return 'Record item not found';
			case 'recordItems.deleteTitle': return 'Delete Record Item';
			case 'recordItems.deleteConfirmation': return 'Are you sure you want to delete this record item?';
			case 'recordItems.deleteSuccess': return 'Record item deleted successfully';
			case 'recordItems.deleteError': return 'An error occurred while deleting';
			default: return null;
		}
	}
}
