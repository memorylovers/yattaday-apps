...
class RecordItemsListPage extends ConsumerWidget {
        ...
        error: (error, stackTrace) => Center(
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () => ref.refresh(watchRecordItemsProvider(userId)),
  ...
  void _navigateToDetail(BuildContext context, RecordItem item) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('${item.title}の詳細画面へ遷移')),
    );
  }
  ...
  void _navigateToEdit(BuildContext context, RecordItem item) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('${item.title}の編集画面へ遷移')),
    );
  }
  ...
  void _navigateToCreate(BuildContext context) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('作成画面へ遷移')),
    );
  }
  ...
  void _showDeleteConfirmDialog(
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('${item.title}の削除確認ダイアログ表示')),
    );
  }
}